...
resource "null_resource" "force-delete-vmss" {
  ...
  provisioner "local-exec" {
    when    = destroy
    command = "if \\[ -x "$(command -v az)" \\]; then az vmss delete --name ${self.triggers.vmss_name} --resource-group ${self.triggers.rg_name} --force-deletion true --subscription ${self.triggers.subscription_id}; fi"
  }
